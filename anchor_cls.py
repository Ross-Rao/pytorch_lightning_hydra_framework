...
from module.contrastive_cluster import ExampleModule
...
@hydra.main(
    ...
    config_name="contrastive_cluster",
)
...
def main(cfg: DictConfig):
    ...
    def visualize_cluster(save_dir):
        ...
        os.makedirs(save_path, exist_ok=True)
        cluster = m0.visualize_cluster(image, label, save_path=save_path)
        ...
        # save tset result to CSV file
        # cluster_df.tail(len(datamodule.test_dataset) * c).to_csv(
        #     os.path.join(save_path, 'cluster_with_risk.csv'), index=False)
        cluster_df.to_csv(os.path.join(save_path, 'cluster_with_risk.csv'), index=False)

        # 绘制每个 cluster 的标签分布饼图
        ...
        # 高风险组
        high_risk_df = cluster_df[cluster_df['risk_group'] == 'High Risk']
        ...
        # 中等风险组
        medium_risk_df = cluster_df[cluster_df['risk_group'] == 'Medium Risk']
        ...
        # 低风险组
        low_risk_df = cluster_df[cluster_df['risk_group'] == 'Low Risk']
    ...
    def visualize_grad(save_dir):
        ...
        for class_label in torch.unique(label).tolist():
            ...
            grad_cam(m, m.model0.unet.down3, class_images, class_labels, save_path=save_path)
        m0.eval()
        m1.eval()
    ...
    def visualize_masks(test_pl_trainer):
        ...
        class MaskReconstruction(pl.LightningModule):
            ...
            def forward(self):
                ...
                hid_x, x_hat = self.unet(self.img * mask)
                hid_x = self.pool(hid_x).squeeze()
                logit_pred = self.act(self.fc(hid_x))
                return logit_pred, x_hat
            ...
            def training_step(self, batch, batch_idx):
                ...
                # total_loss = loss_x + loss_logit + torch.clamp((mask > self.threshold).float().mean() - (1 - self.threshold), min=0)
                self.log("train_loss", total_loss, prog_bar=True)
                self.log("val_loss", total_loss, prog_bar=True)
            ...
            def visualize_masks(self):
                ...
                # masks = torch.where(masks > self.threshold, 1.0, 0.0)
    ...
    if cfg.get('test_mode', False):
        ...
        if os.path.isdir(trial_dir):
            ...
            if cfg.get('visual', False):
                ...
                # statistic_test(trainer.logger.log_dir)

...
from models.resnet_transpose_sequential import ResNet18UpSampleSequentialBackbone
...
__all__ = ['Stage1ModelNoUNet', 'Stage1ModelLossNoUNet', 'Stage2ModelNoUNet', 'Stage2ModelLossNoUNet']
...
class MemoryCluster(nn.Module):
    ...
    def visualize_cluster(self, npc_x, label, save_path, random_state=42):
        ...
        for idx, row in label_1_proportion.iterrows():
            ...
            if idx < 1:  # 前两个数量最多的 cluster 为高风险
...
class Stage1ModelNoUNet(nn.Module):
    def __init__(self,
                 n_samples,
                 neighbors=5,
                 n_cluster=10,
                 ...
                 hidden_dim=512,
                 npc_dim=128,
                 perceptual_loss_ratio=2,
                 activation='ReLU'):
        super().__init__()
        self.encoder_in_channels = encoder_in_channels
        # stage 1
        self.encoder = ResNetEncoder(backbone=encoder, in_channels=encoder_in_channels,
                                     pretrained=True, freeze_all=False)
        # self.decoder = SimpleConvDecoder(in_channels=hidden_dim, out_channels=encoder_out_channels,
        #                                  layers=decoder_layers)
        self.decoder = ResNet18UpSampleSequentialBackbone(in_channels=hidden_dim, out_channels=encoder_out_channels)
...
class Stage1ModelLossNoUNet(nn.Module):
    def __init__(self):
        super().__init__()
...
class Stage2ModelNoUNet(nn.Module):
    def __init__(self,
                 n_cluster=10,
                 seq_len=3,
                 num_classes=3,
...
class Stage2ModelLossNoUNet(nn.Module):
